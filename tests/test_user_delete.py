...
# 3. Третий - негативный, попробовать удалить пользователя, будучи авторизованными другим пользователем
    def test_delete_user_auth_as_another_user(self):
        ...
        response = requests.post('https://playground.learnqa.ru/api/user/', data=register_test_data)
        ...
        # DELETE
        response3 = requests.delete(f"https://playground.learnqa.ru/api/user/{user_id_1st}",
        ...
        # GET
        response4 = requests.get(f"https://playground.learnqa.ru/api/user/{user_id_1st}")

        Assertions.assert_code_status(response4, 200)

        response5 = requests.get(f"https://playground.learnqa.ru/api/user/{user_id_2nd}")
